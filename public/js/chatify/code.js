...
function setContactsLoading(loading = false) {
  if (!loading) {
    $(".listOfContacts").find(".loading-contacts, .error-message").remove();
  } else {
    $(".listOfContacts").find(".error-message").remove();
...
function showContactsError(message) {
  $(".listOfContacts").find(".error-message").remove();
  $(".listOfContacts").prepend(`
    <div class="error-message alert alert-warning" style="margin: 10px; padding: 10px; border-radius: 5px;">
      <i class="fas fa-exclamation-triangle"></i> ${message}
      <button type="button" onclick="retryGetContacts()" class="btn btn-sm btn-link" style="padding: 0 5px;">
...
function retryGetContacts() {
  if (retryCount < MAX_RETRIES) {
    retryCount++;
    getContacts();
  } else {
    showContactsError("Gagal memuat kontak setelah beberapa percobaan. Silakan muat ulang halaman.");
    retryCount = 0;
  }
}
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (response && response.contacts) {
          if (contactsPage < 2) {
            $(".listOfContacts").html(response.contacts);
          } else {
            $(".listOfContacts").append(response.contacts);
          }
          ...
          // Handle pagination
          noMoreContacts = contactsPage >= (response.last_page || 1);
          if (!noMoreContacts) {
            contactsPage++;
            // Auto-load next page if near bottom
            if ($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
              getContacts();
            }
          }
        } else {
          showContactsError("Format respons tidak valid. Silakan coba lagi.");
        }
      },
      error: (xhr, status, error) => {
        ...
        let errorMessage = "Terjadi kesalahan saat memuat kontak";
        if (status === "timeout") {
          errorMessage = "Waktu tunggu habis. Memeriksa koneksi Anda...";
        ...
        if (retryCount < MAX_RETRIES) {
          showContactsError(`${errorMessage} (Percobaan ${retryCount + 1}/${MAX_RETRIES})`);
          setTimeout(() => retryGetContacts(), 2000); // Auto-retry after 2 seconds
        } else {
          showContactsError(errorMessage);
        }
      }
    });
  }
...
// Add scroll event for infinite loading
