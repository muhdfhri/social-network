...
$formattedTime = TimeHelper::timeAgo($created_at);
$timeAndSeen = "<span data-time='$created_at' class='message-time'>
        ".($isSender ? "<span class='fas fa-$seenIcon' seen'></span>" : '' )." <span class='time'>$formattedTime</span>
    </span>";
?>
...
<div class="message-card @if($isSender) mc-sender @endif" data-id="{{ $id }}">
    ...
    <div class="message-card-content">
        ...
        <div class="image-wrapper" style="text-align: {{$isSender ? 'end' : 'start'}}">
            <div class="image-file chat-image" style="background-image: url('{{ asset('storage/'.config('chatify.attachments.folder').'/'.$attachment->file) }}')">
