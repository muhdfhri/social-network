    ...
    <section class="relative block h-1/2">
        @if ($user->thumbnail)
            <div class="absolute top-0 w-full h-full bg-center bg-cover"
                style="background-image: url('{{ $user->thumbnail ? asset('storage/' . $user->thumbnail) : 'https://picsum.photos/id/237/200/300' }}'); background-size:cover; background-repeat:no-repeat;">
            @else
                <div class="absolute top-0 w-full h-full bg-center bg-cover"
                    style="background-image: url('https://picsum.photos/id/237/200/300'); background-size:cover; background-repeat:no-repeat;">
        @endif
                    ...
                    <div class="flex flex-wrap justify-center">
                        <div class="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
                            <div class="relative">
                                ...
                                @else
                                    <div class="shadow-xl rounded-full align-middle border-none absolute -m-16 -ml-20 lg:-ml-16 w-40 h-40 bg-gray-300 flex items-center justify-center">
                                        <span class="text-gray-500 text-4xl">{{ strtoupper(substr($user->first_name, 0, 1)) }}{{ strtoupper(substr($user->last_name, 0, 1)) }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="flex flex-wrap">
                    <div class="w-full  lg:w-40 p-4">
                        <div
                            ...
                            <div class="p-6 text-2xl font-semibold">Teman ({{ $user->numOfFriends }})</div>
                                    ...
                                    <div
                                        ...
                                        @if($friend->profile)
                                            <img class="h-24 min-w-full object-cover"
                                                src="{{ asset('storage/' . $friend->profile) }}" alt="{{ $friend->username }}">
                                        @else
                                            <div class="h-24 w-full bg-gray-300 flex items-center justify-center">
                                                <span class="text-gray-500 text-2xl">
                                                    {{ strtoupper(substr($friend->first_name, 0, 1)) }}
                                                    {{ strtoupper(substr($friend->last_name, 0, 1)) }}
                                                </span>
                                            </div>
                                        @endif
                            ...
                            <div class="flex">
                                ...
                                @else
                                    <div class="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center mr-4">
                                        <span class="text-gray-500">{{ strtoupper(substr($user->first_name, 0, 1)) }}{{ strtoupper(substr($user->last_name, 0, 1)) }}</span>
                                    </div>
                                @endif
                                <div>
        ...
        @else
            <div class="flex items-center justify-center h-32">
                <div class="text-center">
                    <h1 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Tidak Ada Postingan yang Ditemukan</h1>
                    <p class="text-gray-500 dark:text-gray-300 mt-2">Tidak ada postingan yang ditemukan. Silakan periksa kembali nanti.</p>
                </div>
            </div>
        @endif
    </section>
