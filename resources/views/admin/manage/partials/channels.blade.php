<div class="overflow-x-auto shadow-sm rounded-lg">
    @if($channels->count() > 0)
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach($channels as $channel)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                        <!-- Kolom Nama Saluran dengan layout yang diperbaiki -->
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3">
                                <!-- Logo saluran dengan ukuran konsisten -->
                                <div class="flex-shrink-0">
                                    @if($channel->icon)
                                        <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200 dark:border-gray-600" 
                                             src="{{ asset('images/pages/' . $channel->icon) }}" 
                                             alt="{{ $channel->name }}"
                                             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                        <!-- Fallback logo jika gambar gagal load -->
                                        <div class="h-12 w-12 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-semibold text-lg border-2 border-gray-200 dark:border-gray-600" style="display: none;">
                                            {{ strtoupper(substr($channel->name, 0, 1)) }}
                                        </div>
                                    @else
                                        <div class="h-12 w-12 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-semibold text-lg border-2 border-gray-200 dark:border-gray-600">
                                            {{ strtoupper(substr($channel->name, 0, 1)) }}
                                        </div>
                                    @endif
                                </div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-3">
                                <div class="flex-shrink-0">
                                    @if($channel->user->profile || $channel->user->thumbnail)
                                        <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200 dark:border-gray-600" 
                                             src="{{ asset($channel->user->profile ? 'storage/' . $channel->user->profile : 'storage/' . $channel->user->thumbnail) }}" 
                                             alt="{{ $channel->user->first_name }} {{ $channel->user->last_name }}"
                                             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                        ...
                                        <div class="h-12 w-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-semibold text-lg border-2 border-gray-200 dark:border-gray-600" style="display: none;">
                                            {{ strtoupper(substr($channel->user->first_name, 0, 1)) }}
                                        </div>
                                    @else
                        ...
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-800 dark:text-purple-200">
                                ...
                                {{ number_format($channel->followers_count) }} Pengikut
                            </span>
                        </td>
                        
                        <!-- Kolom Tanggal -->
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-xs text-gray-500 dark:text-gray-400" title="{{ $channel->created_at->format('d F Y H:i:s') }}">
                                {{ $channel->created_at->locale('id')->diffForHumans() }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center space-x-2">
                                ...
                                <form action="{{ route('admin.channels.delete', $channel->id) }}" method="POST" class="inline-block delete-channel-form">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 rounded-b-lg">
            {{ $channels->appends(['type' => 'channels'])->links() }}
        </div>
    @else
            ...
            <div class="p-12 text-center">
                <div class="mx-auto h-24 w-24 text-gray-400 dark:text-gray-500 mb-4">
                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="h-full w-full">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2h4a1 1 0 110 2h-1v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6H3a1 1 0 110-2h4zM6 6v14h12V6H6zM8 8v2a1 1 0 102 0V8a1 1 0 10-2 0zm4 0v2a1 1 0 102 0V8a1 1 0 10-2 0z"/>
                    </svg>
                </div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Tidak ada saluran</h3>
                <p class="text-gray-500 dark:text-gray-400">Belum ada saluran yang ditemukan dalam sistem.</p>
            </div>
        </div>
    @endif
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    const channelForms = document.querySelectorAll(".delete-channel-form");
    channelForms.forEach(form => {
        form.addEventListener("submit", function(e) {
            e.preventDefault();
            Swal.fire({
                title: "Yakin hapus saluran ini?",
                text: "Semua data saluran, postingan, dan komentar di dalamnya akan dihapus permanen!",
                icon: "warning",
